"""
Mortar Strength Sub-Skill - 数据解析与生成
专门负责砂浆强度检测描述
"""
...
async def parse_mortar_strength(
    ...
    """
    # 1. 从数据库读取砂浆数据
    records = await _fetch_mortar_data(project_id, node_id)
    
    if not records:
        logger.info(f"Project {project_id}, Node {node_id}: 未找到砂浆强度数据")
        return {
            "has_data": False,
    ...
    if validation["warnings"]:
        logger.warning(f"砂浆数据验证警告: {validation['warnings']}")
...
async def _fetch_mortar_data(project_id: str, node_id: str) -> List[Dict[str, Any]]:
    """从professional_data表获取砂浆数据"""
...
def _validate_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if data["test_count"] < 1:
        warnings.append("检测测点数量少于1")
    
    # 检查强度范围
    if data["avg_strength"] < 1.0 or data["avg_strength"] > 30.0:
        warnings.append(f"平均强度{data['avg_strength']}MPa超出常见范围(1~30MPa)")
    
    # 检查强度等级匹配
    if data.get("strength_grade"):
        import re
        match = re.match(r"^M(\d+(?:\.\d+)?)$", data["strength_grade"])
        if match:
...
def _generate_content(data: Dict[str, Any]) -> str:
    ...
    # 第1部分：检测概述
    overview = f"砂浆强度采用{data['test_method']}检测，检测{data['test_count']}个测点"
    ...
    if data.get("strength_range") and data["test_count"] > 1:
        ...
        result_parts.insert(0, f"砂浆强度推定值{range_str}")
